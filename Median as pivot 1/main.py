'''
    ...
    Median of 3 values (first, middle, last elements of array) as pivot
...
#Формується список з першого, середнього, останього елементів масиву, сортується
#Повертається індекс середнього елементу цього списку в основному масиві А
def Median(input_list, start, end):
    temp = [
        input_list[start],
        input_list[end],
        input_list[int((start+end) / 2)]
    ]
    temp.sort()
    index = input_list.index(temp[1])
    return index
